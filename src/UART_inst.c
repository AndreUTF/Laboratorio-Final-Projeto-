...
osThreadId_t thread1a_id, thread2a_id;
osMutexId_t uart_id;
...
__NO_RETURN void thread2(void *arg){
 ...
 bool esquerdo_up = false;
 bool esquerdo_down = false;
void thread1a(void *arg){
  uint32_t count = 0;
  uint32_t tick;
  char str[30];
  uint32_t str1 = 1;
  uint32_t str2 = 2;
  while(1){
    int s1 = UARTRxBytesAvail();
    int s2 = UARTTxBytesFree();
    int size = UARTgets(str, 13);
    //printf("%d - %d - %d\n", s1, s2, size);
    if(str == "initialized")
    {
      printf("1\n");
...
void main(void){
  ...
  thread1a_id = osThreadNew(thread1a, NULL, NULL);
  //thread2a_id = osThreadNew(thread2a, NULL, NULL);
